...
# =====================================================
# 2. Evaluate Model with 10-fold CV
# =====================================================
...
def select_top_chemicals_and_pathways(X_combined, Y_pathways_log,
    ...
    tunes an XGBoost model via GridSearchCV to predict the target (delta Shannon),
    and evaluates using 10-fold CV. Plots feature importances.
    """
...
# =====================================================
# 6. Network Analysis for ARG EDA
# =====================================================
...
def main():
    ...
    CONFIG_PATH = "F:/ERAEC_ARGs/config.yaml"
    ...
    arg_df   = pd.read_excel(arg_path, index_col=0)  # ARG abundance (CPM)
    shannon_df = pd.read_excel(shannon_path, index_col=0)  # Shannon index for ARG diversity
    
    # -------------------------------
    # Use pre-calculated Delta Shannon from the Shannon file
    # -------------------------------
    ...
    # -------------------------------
    # Task-Specific Modeling: Predict Delta Shannon from Contaminants
    # -------------------------------
    ...
    # -------------------------------
    arg_df_subset = arg_df.loc[X_clean.index]  # Align ARG data to the samples in X_clean
    mean_abundances = arg_df_subset.mean(axis=0)
